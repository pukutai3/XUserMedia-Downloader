...
async function gql(path,variables,features=FEATURES,fieldToggles){const query=new URLSearchParams({variables:JSON.stringify(variables),features:JSON.stringify(features)});if(fieldToggles)query.set("fieldToggles",JSON.stringify(fieldToggles));const response=await fetch(`${path}?${query}`,{credentials:"include",headers:headers()});if(response.status===429){const error=Error("Xの取得制限に達しました。");error.code="RATE_LIMIT";const reset=Number(response.headers.get("x-rate-limit-reset"));if(reset)error.retryAt=new Date(reset*1000);throw error;}if(!response.ok)throw Error(`Xから取得できませんでした (HTTP ${response.status})`);const json=await response.json();if(json.errors?.length)throw Error(json.errors.map(e=>e.message).join(" / "));return json;}
...
function mediaItems(tweet,target){const legacy=tweet.legacy||{},author=tweet.core?.user_results?.result||{},authorLegacy=author.legacy||{},authorCore=author.core||{};const authorId=String(legacy.user_id_str||author.rest_id||""),authorName=String(authorLegacy.screen_name||authorCore.screen_name||"").toLowerCase();if(target.userId?authorId!==String(target.userId):authorName!==target.handle.toLowerCase())return[];const items=legacy.extended_entities?.media||[];return items.map((media,index)=>{let url,ext;if(media.type==="photo"){const raw=media.media_url_https||media.media_url;if(!raw)return null;ext=(raw.match(/\.([a-zA-Z0-9]+)$/)||[])[1]||"jpg";url=`${raw}?format=${ext}&name=orig`;}else{const variants=(media.video_info?.variants||[]).filter(v=>v.content_type==="video/mp4"&&v.url).sort((a,b)=>(b.bit_rate||0)-(a.bit_rate||0));if(!variants.length)return null;url=variants[0].url;ext="mp4";}return{key:`${tweet.rest_id}-${media.id_str||media.media_key||index}`,name:`${tweet.rest_id}_${String(index+1).padStart(2,"0")}.${ext}`,url,type:media.type,tweetId:tweet.rest_id,createdAt:legacy.created_at||null,postUrl:`https://x.com/i/status/${tweet.rest_id}`};}).filter(Boolean);}
async function collectSearch(handle){const found=new Map(),cursors=new Set();let cursor="",page=0;do{if(aborted)throw Error("処理を中止しました。");page++;setStatus("検索からメディア一覧を取得中",`${page}ページ / ${found.size}件`,Math.min(35,page));const variables={rawQuery:`from:${handle} filter:media -filter:retweets`,count:20,querySource:"typed_query",product:"Media",withGrokTranslatedBio:false};if(cursor)variables.cursor=cursor;const batch=scanTimeline(await gql(API.search,variables));for(const tweet of batch.tweets)for(const item of mediaItems(tweet,{handle}))if(!found.has(item.key))found.set(item.key,item);if(!batch.cursor||cursors.has(batch.cursor))break;cursors.add(batch.cursor);cursor=batch.cursor;await new Promise(r=>setTimeout(r,650));}while(page<1000);return[...found.values()];}
async function collectMedia(handle){const userFeatures={hidden_profile_subscriptions_enabled:true,payments_enabled:false,rweb_xchat_enabled:false,profile_label_improvements_pcf_label_in_post_enabled:true,rweb_tipjar_consumption_enabled:true,verified_phone_label_enabled:false,highlights_tweets_tab_ui_enabled:true,responsive_web_twitter_article_notes_tab_enabled:true,subscriptions_feature_can_gift_premium:true,creator_subscriptions_tweet_preview_api_enabled:true,responsive_web_graphql_skip_user_profile_image_extensions_enabled:false,responsive_web_graphql_timeline_navigation_enabled:true,subscriptions_verification_info_is_identity_verified_enabled:true,subscriptions_verification_info_verified_since_enabled:true};const user=findUser(await gql(API.user,{screen_name:handle,withGrokTranslatedBio:false},userFeatures,{withAuxiliaryUserLabels:true}));const userId=user.rest_id,found=new Map(),cursors=new Set();let cursor="",page=0;do{if(aborted)throw Error("処理を中止しました。");page++;setStatus("メディア欄から取得中",`${page}ページ / ${found.size}件`,Math.min(35,page));const variables={userId,count:20,includePromotedContent:false,withClientEventToken:false,withBirdwatchNotes:false,withVoice:true};if(cursor)variables.cursor=cursor;const batch=scanTimeline(await gql(API.media,variables,FEATURES,{withArticlePlainText:false}));for(const tweet of batch.tweets)for(const item of mediaItems(tweet,{handle,userId}))if(!found.has(item.key))found.set(item.key,item);if(!batch.cursor||cursors.has(batch.cursor))break;cursors.add(batch.cursor);cursor=batch.cursor;await new Promise(r=>setTimeout(r,650));}while(page<1000);return[...found.values()];}
async function collect(handle){let searchError;try{const items=await collectSearch(handle);if(items.length)return{items};}catch(error){searchError=error;setStatus("別の取得経路へ切り替え中","検索APIを利用できなかったため、メディア欄を試します。",8);}try{return{items:await collectMedia(handle)};}catch(error){if(error.code==="RATE_LIMIT"&&error.retryAt){error.message+=` ${error.retryAt.toLocaleTimeString()}以降に再実行してください。`;}else if(error.code==="RATE_LIMIT"&&searchError?.code==="RATE_LIMIT"){error.message+=" 検索とメディア欄の両方が制限されています。15分ほど置いて再実行してください。";}throw error;}}
